...
<div class="max-w-7xl mx-auto pb-10 relative">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 animasi-masuk">
        <div>
            <a href="{{ route('admin_bidang.beranda') }}" class="text-sm font-bold text-pupr-blue hover:underline mb-2 inline-block"><i class="fas fa-arrow-left mr-1"></i> Kembali ke Dashboard</a>
                    ...
                    <div class="relative inline-block">
                        <form action="{{ route('admin_bidang.profil.foto') ?? '#' }}" method="POST" enctype="multipart/form-data" id="form-foto">
                            ...
                            <label for="foto_profil" class="cursor-pointer group relative block">
                                <div class="w-28 h-28 bg-white rounded-full p-1 shadow-md border border-gray-100 relative overflow-hidden transition transform hover:scale-105">
                                    @if($user->foto_profil)
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 relative animasi-masuk delay-2">
                ...
                <div class="space-y-5">
                    <div class="flex items-start group">
                        <div class="w-10 h-10 rounded-lg bg-blue-50 text-pupr-blue flex items-center justify-center mr-4 shrink-0 group-hover:bg-pupr-blue group-hover:text-white transition"><i class="fas fa-envelope"></i></div>
                        <div>
                    ...
                    <div class="flex items-start group">
                        <div class="w-10 h-10 rounded-lg bg-green-50 text-green-500 flex items-center justify-center mr-4 shrink-0 group-hover:bg-green-500 group-hover:text-white transition"><i class="fab fa-whatsapp text-xl"></i></div>
                        <div>
                    ...
                    <div class="flex items-start group">
                        <div class="w-10 h-10 rounded-lg bg-blue-50 text-pupr-blue flex items-center justify-center mr-4 shrink-0 group-hover:bg-pupr-blue group-hover:text-white transition"><i class="fas fa-building"></i></div>
                        <div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="kantor_wilayah" value="{{ $user->kantor_wilayah ?? 'Bidang SIGAP Subang' }}" class="w-full border border-gray-300 rounded-lg p-3 text-sm focus:border-pupr-blue outline-none shadow-sm">
                        </div>
                    </div>
                </div>
...
<div id="modal-log" class="fixed inset-0 z-[2000] hidden">
    ...
    <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-3xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div class="p-6 overflow-y-auto flex-1">
            <ul class="space-y-5 border-l-2 border-blue-100 ml-3">
                @forelse($semua_aktivitas ?? [] as $log)
                <li class="relative pl-6 hover:bg-gray-50 p-2 rounded-r-lg transition flex justify-between items-start group">
                    <div>
                        <span class="absolute -left-[10px] top-3.5 w-4 h-4 rounded-full bg-pupr-blue border-4 border-white shadow-sm"></span>
                    ...
                    <form action="{{ route('admin_bidang.profil.log.hapus_satu', $log->id) }}" method="POST" class="opacity-0 group-hover:opacity-100 transition">
                        @csrf
                        @method('DELETE')
                        <button type="button" onclick="konfirmasiHapusSatuLog(this)" class="w-8 h-8 rounded text-gray-300 hover:bg-red-50 hover:text-red-500 flex items-center justify-center transition" title="Hapus catatan ini">
...
<script>
    ...
    function konfirmasiHapusSatuLog(btn) {
        ...
        Swal.fire({
            ...
            text: "Catatan aktivitas ini akan dihapus dari database.",
            icon: 'question',
    ...
    @if(session('sukses'))
        document.addEventListener('DOMContentLoaded', function() {
            Swal.fire({ icon: 'success', title: 'Berhasil!', text: '{{ session('sukses') }}', showConfirmButton: false, timer: 2500, customClass: { popup: 'rounded-2xl shadow-xl' }});
        });
    @endif
</script>
@endpush
